...
namespace Bancario\Modules\Logic;
...
class Moneys
{
    protected $userID         = "";               // new user, it will autogenerate userID and save in cookies
    protected $userFormat     = "COOKIE";         // save userID in cookies (or you can use IPADDRESS, SESSION)
    protected $orderID        = "signuppage";     // Registration Page   
    protected $amountUSD      = 1;                // price per registration - 1 USD
    protected $period         = "NOEXPIRY";       // one time payment for each new user, not expiry
    protected $def_language   = "en";             // default Payment Box Language
    protected $def_payment    = "bitcoin";        // default Coin in Payment Box
    // List of coins that you accept for payments
    // For example, for accept payments in bitcoins, dogecoins use - $available_payments = array('bitcoin', 'dogecoin'); 
    protected $available_payments = array('bitcoin', 'bitcoincash', 'bitcoinsv', 'litecoin', 'dash', 'dogecoin', 'speedcoin', 'reddcoin', 'potcoin', 
     ...
     */
    static function accountNumberToDatabase($string)
    {
        return $string;
    }
    ...
    /**
     * Caso não implementado, então é impossivel gerar uma conta nesse dinheiro
     * usando um token
     */
    static function newPublicKeyFromPrivateKey(string $privateKeyToken)
    {
        return false;
    }
     ...
     * Exemplos
     */
    public function pagueParaVerPagina()
    {
