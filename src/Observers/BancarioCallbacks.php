...
class BancarioCallbacks
{
    ...
    public function handle($event, $payload)
    {
        ...
        // Ignore
        if (!app()->bound(BancarioService::class)) {
            return true;
        }
        ...
        if ($method == 'onBooting') {
            $model::addGlobalScope(new BancarioScope);
            return true;
        }

        if ($method == 'onCreating') {
            $business = app()->make(BancarioService::class);
